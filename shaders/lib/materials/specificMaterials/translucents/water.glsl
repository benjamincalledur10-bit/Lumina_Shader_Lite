// ============================== Step 1: Color Prep ============================== //
        ...
        glColorM = sqrt1(glColorM) * vec3(0.85, 1.08, 1.32);
    #else
        vec3 glColorM = vec3(0.22, 0.62, 0.88);
    #endif

    // Smooth stylized water base (like Butter shader) - no blocky vanilla tile textures!
    vec3 colorPM = vec3(0.35);
    color.rgb = 0.55 * glColorM;
#else
    #if WATER_STYLE < 3
...
#define PHYSICS_OCEAN_INJECTION
    ...
    reflectMult = 1.0;
        ...
        vec3 normalMap = vec3(0.0, 0.0, 1.0);
            ...
            #define WATER_BUMPINESS_M WATER_BUMPINESS * 0.8
                ...
                vec2 normalSmall = vec2(0.0);
                #if DETAIL_QUALITY == 0
                    normalMap.xy = normalMed * WATER_BUMP_MED + normalBig * WATER_BUMP_BIG;
                #else
                    normalSmall = texture2D(gaux4, waterPosM * 4.0 - 2.0 * wind).rg - 0.5;
    ...
    // ============================== Step 3: Water Material Features ============================== //
        ...
        if (isEyeInWater != 1) {
            ...
            // Uniform stylized transparency (no vanilla tile noise)
            color.a = 0.52;
    ...
    color.a = mix(color.a, 1.0, fresnel4);
            ...
            highlightMult = mix(pow2(pow2(highlightMult * 1.1)), 1.0, min1(sqrt(miplevel) * 0.45)) * 0.28;
        #else
            smoothnessG = 0.95;

            highlightMult = min(pow2(pow2(0.35 * 0.4)), 0.5);
