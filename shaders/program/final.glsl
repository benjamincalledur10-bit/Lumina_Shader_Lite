...
void main() {
    ...
    // Balance de Blancos (6500K) / Natural Color Adjustment
    color *= vec3(0.97, 0.99, 1.03);
    ...
    #if LUMINA_SHADER > 0
        // Atmospheric Flicker
        float dreadFlicker = fract(sin(frameTimeCounter * 12.0) * 43758.5453);
        color *= 0.94 + 0.06 * dreadFlicker;
        
        // Noise/Grain
        float dreadNoise = fract(sin(dot(texCoordM, vec2(12.9898, 78.233))) * 43758.5453);
        color += (dreadNoise - 0.5) * 0.03;

        // Desaturation
        float dreadLuminance = dot(color, vec3(0.2126, 0.7152, 0.0722));
        color = mix(color, vec3(dreadLuminance), 0.4);
        
        // Vignette Intensification
        vec2 dreadVigCoord = texCoordM - 0.5;
        color *= 1.0 - dot(dreadVigCoord, dreadVigCoord) * 0.8;
    #endif
