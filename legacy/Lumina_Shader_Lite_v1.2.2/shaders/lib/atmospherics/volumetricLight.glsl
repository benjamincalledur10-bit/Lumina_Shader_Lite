...
vec4 GetVolumetricLight(inout vec3 color, inout float vlFactor, vec3 translucentMult, float lViewPos0, float lViewPos1, vec3 nViewPos, float VdotL, float VdotU, vec2 texCoord, float z0, float z1, float dither) {
    ...
    float vlMult = 1.0 - maxBlindnessDarkness;
        ...
        vlMult *= mix(invNoonFactor2 * 0.875 + 0.125, 1.0, max(vlSceneIntensity, rainFactor2));
            ...
            int sampleCount = vlSceneIntensity < 0.5 ? 15 : 25;
        #elif LIGHTSHAFT_QUALI == 3
            int sampleCount = vlSceneIntensity < 0.5 ? 8 : 15;
        #elif LIGHTSHAFT_QUALI == 2
            int sampleCount = vlSceneIntensity < 0.5 ? 2 : 5;
        #elif LIGHTSHAFT_QUALI == 1
            int sampleCount = vlSceneIntensity < 0.5 ? 3 : 6;
        #endif
    ...
    vec3 rayStartWorld = (gbufferModelViewInverse * vec4(0.0, 0.0, 0.0, 1.0)).xyz;
    vec3 rayDirWorld = (gbufferModelViewInverse * vec4(nViewPos, 0.0)).xyz;
    #if SHADOW_QUALITY > -1
    ...
    for (int i = 0; i < sampleCount; i++) {
        ...
        vec3 playerPos = rayStartWorld + rayDirWorld * currentDist;
        #ifdef END
            playerPos *= 512.0 / far;
